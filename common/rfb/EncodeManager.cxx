                                         ...
                                         size_t maxUpdateSize)
{
  doUpdate(false, getLosslessRefresh(req, maxUpdateSize),
           core::Region(), core::Point(), pb, renderedCursor);
}

bool EncodeManager::handleTimeout(core::Timer* t)
{
...
void EncodeManager::doUpdate(bool allowLossy, const core::Region& changed_,
                             const core::Region& copied, const core::Point& copyDelta,
                                         ...
                                         size_t maxUpdateSize)
{
  ...
  while (!rects.empty()) {
    ...
    if ((area + rect.area()) > maxUpdateSize) {
      ...
      refresh.assign_union(core::Region(rect));
      break;
    }
...
Encoder *EncodeManager::startRect(const core::Rect& rect, int type)
{
  ...
  if ((encoder->flags & EncoderLossy) &&
    ...
    lossyRegion.assign_union(core::Region(rect));
  else
    lossyRegion.assign_subtract(core::Region(rect));
  ...
  pendingRefreshRegion.assign_subtract(core::Region(rect));
                                  ...
                                  const PixelBuffer* pb)
{
  ...
  for (dy = rect.tl.y; dy < rect.br.y; dy += SolidSearchBlock) {
    ...
    for (dx = rect.tl.x; dx < rect.br.x; dx += SolidSearchBlock) {
      ...
      pb->getImage(colourValue, core::Rect(dx, dy, dx+1, dy+1));
      ...
      if (checkSolidTile(sr, colourValue, pb)) {
        ...
        changed->assign_subtract(core::Region(erp));
...
bool EncodeManager::checkSolidTile(const core::Rect& r, const uint8_t* colourValue,
                                   const PixelBuffer *pb)
{
  switch (pb->getPF().bpp) {
  case 32:
...
void EncodeManager::extendSolidAreaByPixel(const core::Rect& r, const core::Rect& sr,
                                           const uint8_t* colourValue,
                                           const PixelBuffer *pb, core::Rect* er)
{
  int cx, cy;
