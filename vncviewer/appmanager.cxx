...
AppManager::AppManager()
    : QObject(nullptr), error_(0), facade_(new QVNCConnection), view_(nullptr), scroll_(new QVNCWindow),
      rfbTimerProxy_(new QTimer), visibleInfo_(false)
#if defined(__APPLE__)
      ,
...
void AppManager::openVNCWindow(int width, int height, QString name)
{
    ...
    connect(
        view_,
        &QAbstractVNCView::fullscreenChanged,
        this,
        [this](bool enabled) {
    ...
    if (!ViewerConfig::config()->remoteResize())
    {
        ...
        view_->setFixedSize(QSize(width, height));
    }
...
#if defined(__APPLE__)
void AppManager::openOverlay(QString qml, char const* title, char const* message)
{
