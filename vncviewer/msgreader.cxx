...
static rfb::LogWriter vlog("QMsgReader");
...
bool QMsgReader::readExtendedClipboard(rdr::S32 len)
{
  ...
  if (action & clipboardCaps) {
    ...
    for (i = 0;i < 16;i++) {
      ...
      zis.readBytes(reinterpret_cast<char*>(buffers[num]), lengths[num]);
      num++;
    }
...
bool QMsgReader::readSetXCursor(int width, int height, const Point& hotspot)
{
  ...
  if (width * height > 0) {
    ...
    is->readBytes(reinterpret_cast<char*>(data.buf), data_len);
    is->readBytes(reinterpret_cast<char*>(mask.buf), mask_len);
...
bool QMsgReader::readSetVMwareCursor(int width, int height, const Point& hotspot)
{
  ...
  if (type == 0) {
    ...
    is->readBytes(reinterpret_cast<char*>(andMask.buf), len);
    is->readBytes(reinterpret_cast<char*>(xorMask.buf), len);
    ...
    is->readBytes(reinterpret_cast<char*>(data.buf), width*height*4);
