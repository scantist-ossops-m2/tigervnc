...
bool Win32TouchHandler::processEvent(UINT Msg, WPARAM, LPARAM lParam)
{
  GESTUREINFO gi;
...
void Win32TouchHandler::handleWin32GestureEvent(GESTUREINFO gi)
{
  ...
  if (gi.dwID == GID_BEGIN) {
    ...
    qDebug() << "Win32TouchHandler::handleWin32GestureEvent: gi.dwID= GID_BEGIN: SetCursor() x=" << gi.ptsLocation.x << ", y=" << gi.ptsLocation.y;
    return;
  } else if (gi.dwID == GID_END) {
    gestureActive = false;
    ignoringGesture = false;
    qDebug() << "Win32TouchHandler::handleWin32GestureEvent: gi.dwID= GID_END: gesture ignored";
    return;
  }
  ...
  if (ignoringGesture) {
    qDebug() << "Win32TouchHandler::handleWin32GestureEvent: gi.dwID>=GID_ZOOM: previous gesture canceled";
    return;
  }
  ...
  case GID_PAN:
    if (isSinglePan(gi)) {
      ...
      if (!startedSinglePan &&
        ...
        qDebug() << "Win32TouchHandler::handleWin32GestureEvent: gi.dwID=GID_PAN: gesture unexpectedly ignored";
        return;
      }
                                        ...
                                        const GestureEvent origEvent)
{
  ...
  if (1 <= button && button <= 3) {
    ...
    qDebug() << "Win32TouchHandler::fakeButtonEvent: x=" << origEvent.eventX << ", y=" << origEvent.eventY << ", buttons=" << hex << fakeButtonMask;
#endif
  } else {
    POINT pos;
