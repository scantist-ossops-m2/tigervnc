...
static rfb::LogWriter vlog("DecodeManager");

DecodeManager::DecodeManager(QVNCConnection *conn) :
...
bool DecodeManager::decodeRect(const rfb::Rect& r, int encoding, rfb::ModifiablePixelBuffer* pb)
{
  rfb::Decoder *decoder;
  ...
  if (!rfb::Decoder::supported(encoding)) {
    vlog.error("Unknown encoding %d", encoding);
    throw rdr::Exception("Unknown encoding");
  }

  if (!decoders[encoding]) {
    decoders[encoding] = rfb::Decoder::createDecoder(encoding);
    if (!decoders[encoding]) {
      vlog.error("Unknown encoding %d", encoding);
      throw rdr::Exception("Unknown encoding");
    }
  ...
  bufferStream->clear();
  try {
    if (!decoder->readRect(r, conn->istream(), *conn->server(), bufferStream))
      return false;
  } catch (rdr::Exception& e) {
    throw rdr::Exception("Error reading rect: %s", e.str());
  }
...
void DecodeManager::logStats()
{
  ...
  for (i = 0;i < (sizeof(stats)/sizeof(stats[0]));i++) {
    ...
    rfb::siPrefix(stats[i].rects, "rects", a, sizeof(a));
    rfb::siPrefix(stats[i].pixels, "pixels", b, sizeof(b));
    vlog.info("    %s: %s, %s", rfb::encodingName(i), a, b);
    rfb::iecPrefix(stats[i].bytes, "B", a, sizeof(a));
    vlog.info("    %*s  %s (1:%g ratio)",
              (int)strlen(rfb::encodingName(i)), "",
              a, ratio);
  }

  ratio = (double)equivalent / bytes;

  rfb::siPrefix(rects, "rects", a, sizeof(a));
  rfb::siPrefix(pixels, "pixels", b, sizeof(b));
  vlog.info("  Total: %s, %s", a, b);
  rfb::iecPrefix(bytes, "B", a, sizeof(a));
...
DecodeManager::QueueEntry* DecodeManager::DecodeThread::findEntry()
{
  ...
  for (iter = manager->workQueue.begin();
    ...
    if (entry->decoder->flags & rfb::DecoderOrdered) {
    ...
    // pair of rectangles.
    if (entry->decoder->flags & rfb::DecoderPartiallyOrdered) {
      for (iter2 = manager->workQueue.begin(); iter2 != iter; ++iter2) {
        if (entry->encoding != (*iter2)->encoding)
          continue;
